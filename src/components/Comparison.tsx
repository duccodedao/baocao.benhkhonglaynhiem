...
import { GitCompare, AlertCircle, FileSpreadsheet } from 'lucide-react';
...
export const Comparison: React.FC = () => {
  ...
  const handleExportExcel = () => {
    showToast('Đã xuất báo cáo So sánh số liệu ra tập tin Excel!', 'success');
  };
      ...
      <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <GitCompare className="w-6 h-6 text-rose-600 dark:text-rose-400" />
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">
              So sánh Số liệu Cùng kỳ
            </h2>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            So sánh giữa các tháng, quý hoặc cùng kỳ năm trước
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-12 shadow-sm text-center space-y-3">
        ...
        <div className="max-w-md mx-auto space-y-1">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            So sánh cùng kỳ hiện tại xóa tất cả dòng nội dung, vì chưa triển khai.
          </p>
        </div>
      </div>
    </div>
